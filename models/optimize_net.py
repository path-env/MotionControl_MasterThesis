...
def objective(trial):
    ...
    _, f1_train, f1_val = train_and_validate(data, model, loss, optimizer,LRscheduler, tb, dCfg, nCfg, epochs=EPOCHS, opt_trial=trial)
    if f1_val > best_f1:
        best_f1 = f1_val
        print('Saving the model.....')
        model_scripted = torch.jit.script(model)
        model_scripted.save(model,f"/media/mangaldeep/HDD2/workspace/MotionControl_MasterThesis/models/{model._get_name()+'_optim.pt'}")
    return f1_val


study = optuna.create_study(direction="maximize", sampler=optuna.samplers.TPESampler(),study_name='ATTNnet_Physionet')
